...
def test_list_lid_groups():
    with Simulation(MODEL_LIDS_PATH) as sim:
        ...
        for i, group in enumerate(LidGroups(sim)):
            if i == 0:
                assert (
                    "subcatchment {} has {} lid units".format(group, len(group))
                    == "subcatchment 1 has 0 lid units"
                )
            if i == 1:
                assert (
                    "subcatchment {} has {} lid units".format(group, len(group))
                    == "subcatchment 2 has 3 lid units"
                )
            if i == 2:
                assert (
                    "subcatchment {} has {} lid units".format(group, len(group))
                    == "subcatchment 3 has 0 lid units"
                )
            if i == 3:
                assert (
                    "subcatchment {} has {} lid units".format(group, len(group))
                    == "subcatchment 4 has 0 lid units"
                )
            if i == 7:
                assert (
                    "subcatchment {} has {} lid units".format(group, len(group))
                    == "subcatchment 8 has 0 lid units"
                )
...
def test_lid_group_params():
    with Simulation(MODEL_LIDS_PATH) as sim:
        ...
        for i, step in enumerate(sim):
            if i == 115:
                assert (
                    sim.current_time.strftime("%Y-%m-%d %H:%M:%S")
                    == "1998-01-02 05:00:00"
                )
